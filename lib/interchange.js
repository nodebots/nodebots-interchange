...
Interchange.prototype.install_firmware = async function(firmware, options = {}) {
  ...
  try {
    ...
    await this.flash_firmware(hexpath, options)
      .then(() => {
        console.log('flash completed', tmpdir);
        if (tmpdir) {
...
/**
  try {
    this.check_firmware(firmware, settings, function(hex_path, tmp_dir, options) {
      // change this to this method of calling in a chain.

      this.flash_firmware(hex_path, options)
        .then(() => {
...
Interchange.prototype.flash_firmware = function(firmware, opts) {
  ...
  return new Promise((resolve, reject) => {
    ...
    avrgirl.flash(firmware, (err) => {
      ...
      // this is going to be an issue for follow on. Needs to be sorted to send
      //  the port back in some fashion perhaps.
      if (port == '') {
