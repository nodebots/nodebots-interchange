    ...
    Download(manifest_uri).then(data => {
        ...
        Download(hex_uri).then(hex_data => {
            ...
            try {
                ...
                console.log("Can't write hex file to file system");
                self.clean_tmp_dir(tmp_dir);
            ...
            console.log("Can't download the hex file (%s)", hex_uri);
            self.clean_tmp_dir(tmp_dir);
            throw new Error("HexDownloadError");
        });

    }).catch(function(err) {
        console.log("There was an error downloading the manifest file.");
    });
};
