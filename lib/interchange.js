...
Interchange.prototype.install_firmware = async function(firmware, options = {}) {
  ...
  try {
    ...
    if (!usingFirmata) {
      /* istanbul ignore next */
      if (!process.env.TEST) {
        console.log('optionally configure');
      }
      this.set_firmware_details(port, opts);
    }
  } catch (e) {
    throw e;
  }
...
Interchange.prototype.get_firmware_info = function(port) {
  ...
  return new Promise((resolve, reject) => {
    if (!port) {
      reject(new Error('Please provide a device path'));
    }
    ...
    ic_client.on('ready', () => {
      ic_client.get_info((err, data) => {
        ...
        if (typeof(fw_details) === 'undefined') {
          console.log('Cannot find firmware match from library. Best guess');
          if (data.creatorID == 'undefined') data.creatorID = '0x00';
          fw_details = {
            name: 'Unknown',
...
Interchange.prototype.set_firmware_details = function(port, opts) {
  ...
  ic_client.on('ready', () => {
    ...
    console.log('These are the settings we are about to use', settings);
